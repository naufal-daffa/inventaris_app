...
class _DataBarangPageState extends State<DataBarangPage> {
  ...
  Future<void> _hapusData(int id, BuildContext dialogContext) async {
    try {
      await _itemService.deleteItem(id);
      _showSnackBar(context, "Barang Berhasil Dihapus", isError: false);
    } catch (e) {
      _showSnackBar(context, "Barang Gagal Dihapus ", isError: true);
    }
  }

  void _refreshData() {
    setState(() {
      _itemFuture = _itemService.getItems().then((items) {
    ...
    showDialog(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              // 1. Panggil fungsi hapus dari service Anda
              _hapusData(id, dialogContext);
              _refreshData();
              Navigator.pop(dialogContext);
              // 3. Panggil setState atau refresh data di sini jika diperlukan
            },
            style: ElevatedButton.styleFrom(
              backgroundColor: const Color(0xFFEF4444),
              foregroundColor: Colors.white,
  ...
  Widget build(BuildContext context) {
    Future<List<ItemModel>> itemFuture;
    return Scaffold(
